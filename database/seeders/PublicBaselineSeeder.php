...
class PublicBaselineSeeder extends Seeder
{
    ...
    private function seedLocalAiRows(): void
    {
        ...
        $this->insertIfMissing('llm_instances', ['instance_id' => 'claude_cli'], [
            'instance_name' => 'Claude CLI',
            'instance_type' => 'claude_cli',
            'priority' => 80,
            'is_active' => false,
            'routability' => 'allowed',
            'gpu_target' => 'none',
            'compat_runtime_family' => 'claude_cli',
            'compat_backend' => 'cli',
            'compat_status' => 'provisional',
            'is_healthy' => false,
            'health_score' => 0,
            'capabilities' => json_encode(['text', 'vision', 'tools', 'streaming'], JSON_UNESCAPED_SLASHES),
            'is_censored' => true,
            'supported_models' => json_encode([config('services.anthropic.model', 'claude-sonnet')], JSON_UNESCAPED_SLASHES),
            ...
            'config' => json_encode([
                'models' => [
                    'standard' => config('services.anthropic.model', 'claude-sonnet'),
                    'quality' => config('services.anthropic.model', 'claude-sonnet'),
                ],
            ], JSON_UNESCAPED_SLASHES),
            'notes' => 'Optional external CLI provider. Disabled in the public baseline.',
        ]);
    }
    ...
    private function seedOptionalCloudProviders(): void
    {
        foreach ([
            [
                ...
                'capabilities' => ['text_generation' => true, 'chat' => true, 'sensitive_safe' => true, 'vision' => false, 'embedding' => false, 'tool_calling' => true],
            ...
            [
                ...
                'capabilities' => ['text_generation' => true, 'chat' => true, 'sensitive_safe' => false, 'vision' => true, 'embedding' => false, 'tool_calling' => true],
            ...
            [
                ...
                'capabilities' => ['text_generation' => true, 'chat' => true, 'sensitive_safe' => true, 'vision' => true, 'embedding' => true, 'tool_calling' => true],
            ...
            [
                ...
                'capabilities' => ['text_generation' => true, 'chat' => true, 'sensitive_safe' => false, 'vision' => true, 'embedding' => true, 'tool_calling' => true],
            ...
            $this->insertIfMissing('llm_instances', ['instance_id' => $provider['instance_id']], [
                ...
                'config' => json_encode([
                    ...
                    'notes' => 'Optional public add-on. Disabled until the operator adds a key and enables the row.',
                ], JSON_UNESCAPED_SLASHES),
                'notes' => 'Optional cloud provider. Public baseline leaves this disabled.',
            ]);
        }
    }
