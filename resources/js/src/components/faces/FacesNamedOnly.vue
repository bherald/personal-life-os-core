    ...
    <div class="min-w-0 flex-1 overflow-y-auto p-4">
      <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <span class="text-xs uppercase tracking-wide text-ops-text-muted">
          {{ namedOnlyTotal }} named-only face{{ namedOnlyTotal === 1 ? '' : 's' }}
        </span>
        <button
      ...
      <div v-if="faces.length === 0 && !namedOnlyLoading" class="py-16 text-center text-ops-text-muted">
        <p class="text-lg">No named-only faces</p>
        <p class="mt-1 text-sm opacity-60">Named faces are linked or awaiting new detections</p>
      </div>
