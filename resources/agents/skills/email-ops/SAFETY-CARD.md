...
**Capabilities:** Monitors email pipeline health, bounce rates, delivery metrics, Thunderbird MCP status.

**Constraints:**
- Cannot send emails or modify email rules. Read-only email access via Thunderbird MCP. Privacy-sensitive (sensitive_safe LLMs only).
...
**Kill Switches:**
- Disable job: `UPDATE scheduled_jobs SET enabled=0 WHERE command LIKE '%email-ops%'`
- Clear session lock: `DEL agent_session_lock:email-ops`
