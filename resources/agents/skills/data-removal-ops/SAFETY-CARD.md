...
**Capabilities:** Monitors data removal pipeline health, broker status, relisting detection, request tracking.

**Constraints:**
- Read-only assessment. Cannot submit removal requests or trigger scans. References subjects by ID only (no PII in reports).
...
**Kill Switches:**
- Disable job: `UPDATE scheduled_jobs SET enabled=0 WHERE command LIKE '%data-removal-ops%'`
- Clear session lock: `DEL agent_session_lock:data-removal-ops`
