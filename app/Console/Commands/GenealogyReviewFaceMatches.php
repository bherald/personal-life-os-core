...
class GenealogyReviewFaceMatches extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($approveId) {
            return $this->approveMatch((int) $approveId);
        }

        // Handle reject action
        if ($rejectId) {
            return $this->rejectMatch((int) $rejectId);
        }

        // Handle bulk approve by type
        if ($approveAllType) {
            return $this->approveAllByType($treeId, $approveAllType);
        }
    ...
    private function listMatches(int $treeId, string $status, int $limit): int
    {
        ...
        $statusFilter = $status === 'all' ? '' : "AND q.status = '{$status}'";
        ...
        $matches = DB::select($sql, [$treeId, $limit]);
        ...
        $this->newLine();
        $this->info('Actions:');
        $this->info('  Approve: php artisan genealogy:review-face-matches --approve=ID');
        $this->info('  Reject:  php artisan genealogy:review-face-matches --reject=ID');
        $this->info('  Bulk:    php artisan genealogy:review-face-matches --approve-all-type=typo');
     ...
     */
    private function approveMatch(int $queueId): int
    {
        $match = DB::selectOne(
        ...
        if (!$this->confirm('Confirm approval?', true)) {
            $this->info('Cancelled.');
            return Command::SUCCESS;
        }
     ...
     * Reject a face match.
     */
    private function rejectMatch(int $queueId): int
    {
        $match = DB::selectOne(
        ...
        if (!$this->confirm('Confirm rejection?', true)) {
            $this->info('Cancelled.');
            return Command::SUCCESS;
        }
     ...
     */
    private function approveAllByType(int $treeId, string $matchType): int
    {
        ...
        if (!$this->confirm("Approve all {$count->count} {$matchType} matches?", false)) {
            $this->info('Cancelled.');
            return Command::SUCCESS;
        }
